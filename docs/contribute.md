...
Please help us!  Check out the [Issues](https://github.com/mgmt-sa-tiger-team/sap-hana-workshop/issues) for a list of what we are working on.
...
Make sure you are not behind (in sync) and then submit a PR.  
...
Just because you submit a PR, doesn't mean that it will get accepted.  Right now the QA process is manual for Workshops, so provide detailed directions on
 ...
 - BEST PRACTICE?  Is this the "best" way to do this?  Link to documentation or examples where the way you solved your issue or improve the content.
...
To verify locally, install `tox` and from within your local clone, run `tox -e linters`.
